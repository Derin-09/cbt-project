...
export default function Page({ params }: PageProps) {
  ...
  // recalc score when answers change
  useEffect(() => {
    if (result.length > 0) {
      let newScore = 0
      Object.entries(selectedAnswers).forEach(([qIdx, chosenIdx]) => {
        const question = result[Number(qIdx)]
        if (chosenIdx !== null && question.options[chosenIdx] === question.answer) {
          newScore++
        }
      })
      setScore(newScore)
  ...
  return (
    <main className='hscreen wscreen text-black select-none'>
      <section>
        {/* Header */}
        <div className='flex gap-10 justify-between h-[80px] items-center text-center absolute z-50 top-0 right-10'>
          <div>
            <div className='text-xl'>
              You have: <span className='font-bold'>{formatTime(time)} (mins) left</span>
            </div>
          </div>
          <Link href={'/score'}>
        ...
        <section className='grid grid-cols-3 gap-8 p-8'>
            ...
            <section className="ml-0 col-span-2 max-w-[900px]">
              ...
              <section className="w-full px-5 grid grid-cols-15 gap-2">
                {range.map((num, i) => {
                  ...
                  let bubbleStyle = "bg-gray-100 text-black"
                  if (isAnswered) bubbleStyle = "bg-[#2F4156]/70 text-white"
                  if (isCurrent) bubbleStyle = "bg-[#2F4156] text-white"

                  return (
                    <div
                      key={num}
                      className={`rounded-full p-3 font-bold text-center cursor-pointer mt-5 ${bubbleStyle}`}
              ...
              <section className="px-5 pt-15">
                ...
                <div className="grid grid-cols-2 gap-x-20 justify-between pt-5 w-full">
                  {result[currentIndex].options.map((opt, idx) => {
                    ...
                    return (
                      <div
                        ...
                        className={`mt-5 rounded-lg p-3 hover:cursor-pointer ${isSelected ? 'bg-[#5EA43D] text-white' : ''}`}
                      >
                        {idx + 1}. {opt}
                      </div>
                    )
          ...
          <section className='col-span-1 ml-4 p-5 rounded-xl'>
            ...
            <section className="p-4">
              {/* <p className="font-bold text-xl">Live Score: {score}</p> */}
            </section>
          </section>
        </section>
      </section>
...
//                     className="rounded-3xl bg-[#2F4156] py-3 px-6 text-white"
//                     onClick={() => {
//                       if (opt === result[currentIndex].answer) setScore(+1)
